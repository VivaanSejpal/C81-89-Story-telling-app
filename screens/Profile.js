...
export default class Profile extends Component {
  ...
  toggleSwitch() {
    ...
    If `isEnabled` is`false`, meaning the switch is in the off position, `theme` is set to `"dark"`.
    when, this.state.isEnable = false,
                  NOT false     == true  ? dark: light
    
    If `isEnabled` is `true`, meaning the switch is in the on position, `theme` is set to `"light"`. 
    when, this.state.isEnable = true,
